...
var empAuthModel = new(models.EmpAuthModel)

//TokenValid ...
func (ctl EmpAuthController) TokenValid(c *gin.Context) {

	tokenAuth, err := empAuthModel.ExtractTokenMetadata(c.Request)
	if err != nil {
	...
	userID, err := empAuthModel.FetchAuth(tokenAuth)
	if err != nil {
...
func (ctl EmpAuthController) Refresh(c *gin.Context) {
	var tokenForm forms.FormerToken
	...
	if ok && token.Valid {
		...
		deleted, delErr := empAuthModel.DeleteAuth(refreshUUID)
